...
# Stops only THIS worktree's backend (its port + titled window).
...
Write-Host "Stopping backend server (port $port)..." -ForegroundColor Yellow
...
if ($stillRunning) {
    ...
    Write-Host "No backend server running on port $port." -ForegroundColor Cyan
}
