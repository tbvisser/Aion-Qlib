...
export function MeasurePanel({ expression, context }: {
  ...
  // The IC belongs to the expression it was measured on. When the expression
  // changes — another column opened, or this one edited — the old verdict must
  // go, or column B's inspector shows column A's chart under B's name. The
  // sequence number also retires any answer still in flight.
  const seq = useRef(0)
  useEffect(() => {
    seq.current++
    setResult(null)
  ...
  const measure = async () => {
    ...
    setError(null)
    try {
      const measured = await api.evaluateFactor({
        expression,
      ...
      if (mine === seq.current) setResult(measured)
    } catch (e) {
  ...
  return (
    <Panel title="Measure" hint="before you spend a backtest on it">
      <div className="space-y-2">
        ...
        {blocked && (
          <p className="text-label leading-relaxed text-clay">{blocked}</p>
        )}
        {error && (
          <p className="text-label leading-relaxed text-clay">{error}</p>
        )}
      </div>
      ...
      {result && (
        <div className="mt-3 space-y-2">
          ...
          {verdict && (
            <p className="text-label leading-relaxed text-muted-foreground">{verdict}</p>
          )}
