...
const TEMPLATE_FAMILY_INFO: Record<string, { label: string; color: string }> = {
  baseline: { label: 'Baseline', color: '#34d399' },
  factors: { label: 'Strategy', color: '#34d399' },
  shape: { label: 'Management', color: '#a78bfa' },
  universe: { label: 'Universe', color: '#60a5fa' },
  horizon: { label: 'Horizon', color: '#fbbf24' },
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        {!apiReady && !loading && (
          <p className="px-2 py-2 text-[11px] text-muted-foreground">
            Running offline: connect the backend for the full block library.
          </p>
        )}
                ...
                return (
                  ...
                  >
                    <CollapsibleTrigger asChild>
                      <button className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-[11px] font-medium uppercase tracking-wider text-muted-foreground hover:bg-accent">
                        {open
        ...
        {tab === 'blocks' && (
          <div className="space-y-1">
            {loading && categories.length === 0 ? (
              ...
              <p className="px-2 py-4 text-[11px] text-muted-foreground">
                {search ? 'No blocks match.' : 'No blocks available.'}
              </p>
            ) : (
                          ...
                          return (
                            <li key={meta.id}>
                              <div
                                ...
                                className={cn(
                                  'group flex cursor-grab items-center gap-2 rounded-md border border-transparent px-2 py-1.5 text-left text-[11px] transition-colors hover:border-border/50 hover:bg-accent active:cursor-grabbing',
                                )}
